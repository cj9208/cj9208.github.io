...
- **用不用 rerank？** rerank 是补偿性组件，不是默认组件。上游条件化强（意图识别 + query clean + metadata 过滤检索）时，候选集已经很精准，rerank 边际收益小。原则与 RAG 经济学一致：先修上游，补偿层自然不再必要；语料大、意图混、精度要求极高时再把它当精度杠杆加上。
- **纯 similarity search 不够精准怎么办？** 两层递进：第一层 hybrid search——向量 + metadata/结构化过滤（文档类型、域、权限、时效、来源质量），打分前先收窄候选集；第二层 agent 式监控 + fallback——retrieval 步骤监控实时信号（如 top 结果的相似度分布），显著低于近期均值时在有界 retry 预算内触发改述查询或合成可能答案再检索，保住 recall 而不产生开放式模型循环。
