...
4. **引用位置策略**：
   ...
   **总原则：只有"直接支撑当前论证"的引用才内联；"相关但不是文章主旨"的引用一律放文末「延伸阅读」。**
   - 判断标准：删掉该引用后，当前段落的论证是否受损？受损 → 内联；不受损（仅为背景数据、旁证、同源案例、术语辨析、延伸联想）→ 文末。
   - 文末小节统一命名为 `## 延伸阅读`（体感/叙事类可用 `## 更多阅读`），按主题分组，每条附**一句与本文的逻辑关联**，必要时点明"关联与区分"：
     ...
     **<分组标题>**
     ...
     - [《标题》](https://cj9208.github.io/blog/<目录>/<slug>/)：一句话说明它与本文的逻辑关联（含必要的关联与区分）。
     ```

   **按类型的默认落位：**
   - **论证/分析型、复盘/案例型**：**直接支撑论证**的引用内联；其余（背景数据、旁证、同源案例、术语辨析）放文末「延伸阅读」。不要把"参见 / 同源"式的引用塞进正文打断论证链。
   - **体感/方向型**：默认全部放文末「更多阅读」，避免打断叙述；仅当用户明确要求时才内联。
   - **综述/总纲型**：在正文对应分节内串联引用（其正文本身即由链接构成）。
5. 引用规则（遵循 AGENTS.md）：
...
- **中文标点紧邻 `**` 会破坏加粗（CommonMark flanking 规则）**：加粗分隔符 `**` 只有在满足 left/right-flanking 条件时才生效。若闭合 `**` 前是全角标点（如 `）`、`，`、`：`），而后紧跟的是中文字符（非标点、非空白），则不满足 right-flanking，加粗失效并渲染出字面 `**`。
  - ❌ `**信号筛选（Signal Screening）**问题` → 渲染成字面 `**信号筛选（Signal Screening）**问题`
  - ✅ `**信号筛选**（Signal Screening）：` 或 `**信号筛选（Signal Screening）**。`（闭合 `**` 后接标点即可）
  - 规避：把标点移出加粗范围、让闭合 `**` 后紧跟标点/空白、或重写句子使加粗段以非标点字符收尾。
  - 检测：构建后 grep 渲染 HTML 是否残留字面 `**`，或确认关键术语存在 `<strong>`。
